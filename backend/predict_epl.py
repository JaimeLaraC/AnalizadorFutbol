#!/usr/bin/env python3
"""Predicciones Premier League J10-J15"""
import pandas as pd
import pickle
import numpy as np
...
# 1. Cargar datos
df = pd.read_csv('data/test_epl_2025.csv')
print(f"📊 Total partidos Premier League 2025: {len(df)}")
...
with get_db_session() as db:
    ...
    for j in range(10, 16):
        ...
        print(f"\n{'='*70}")
        print(f"📊 JORNADA {j} - PREMIER LEAGUE 2025/26")
        print("="*70)
        ...
        for i, fid in enumerate(df_j['fixture_id'].values):
            ...
            print(f"{sym} {p['home'][:15]:15} vs {p['away'][:15]:15} | {p['hg']}-{p['ag']} | Real: {real_txt:9} | Pred: {pred_txt:9} | Conf: {conf*100:.0f}%")
        
        total = len(df_j)
        total_aciertos += aciertos
        total_partidos += total
        print(f"\n📈 RESUMEN J{j}: {aciertos}/{total} = {aciertos/total*100:.1f}%")
    
    print(f"\n{'='*70}")
    print(f"🏆 TOTAL PREMIER LEAGUE J10-J15: {total_aciertos}/{total_partidos} = {total_aciertos/total_partidos*100:.1f}%")
    print("="*70)
